...
$button_icon = '<i class="bx bx-cart-alt" title="Add to cart"></i>';
$button_text = esc_html__('', 'fluxor');

// Condizione 1: Prodotto variabile
if ($product_type === 'variable') {
    $button_icon = '<i class="las la-edit" title="Choose options"></i>';
    $button_text = esc_html__('', 'fluxor');
}

// Condizione 2: Prodotto esaurito
if (!$in_stock) {
    $button_icon = '<i class="bx bx-block" title="Not available"></i>';
    $button_text = esc_html__('', 'fluxor');
    $args['class'] .= ' disabled';
}
...
if ($product->is_on_sale() && $in_stock && $product_type === 'simple') {
    $button_icon = '<i class="bx bx-cart-alt" title="Buy now"></i>';
    $button_text = esc_html__('', 'fluxor');
}
